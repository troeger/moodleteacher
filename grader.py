...
def handle_submission(submission):
    ...
    '''
    if not submission.is_empty() and submission.gradingstatus == submission.NOT_GRADED:
        print("#"*78)
...
if __name__ == '__main__': 
    ...
    for assignment in sorted(assignments, key=lambda x:x.deadline):
        print("Checking assignment '{0.name}' in '{0.course}', due to {0.deadline} ...".format(assignment))
        if not assignment.course.can_grade:
            print("  Skipping assignment, you have no rights to grade it.")
            continue
        if assignment.deadline_over() and len(assignment.submissions)>0:
            # Assignment is already closed and we have submissions
